...
public class SearchType extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String type = request.getParameter("type");  // 获取搜索商品类型框中的输入内容
        // 创建根据商品类型筛选商品的 SQL 语句
        String sql = "select distinct(type) from commodity where type like "+"\"%"+type+"%\"";
        Connection conn = new JDBCUtil().getConnection();  // 通过辅助类创建 Connection 连接对象
        List<String> typeList = new ArrayList<>();  // 创建根据输入框的关键字筛选出的所有商品类型的List对象
        ...
        try {
            ...
            while (resultSet.next()) {  // 遍历结果集中的所有查询结果
                typeList.add(resultSet.getString(1));  // 将获取到的数据添加入 typeList 集合中
            }
        ...
        for (int i = 0; i < typeList.size(); i ++) {
            if (i != typeList.size()-1) {
                String addString = "{\"type\":\""+typeList.get(i)+"\"},";
                data += addString;
            }else {
                String addString = "{\"type\":\""+typeList.get(i)+"\"}";
                data += addString;
